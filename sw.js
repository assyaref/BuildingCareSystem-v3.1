...
// Version 2.3.11 FINAL PWA
// ======================================================

"use strict";

const CACHE_VERSION = "v2.3.11";
...
self.addEventListener("push", event => {
    ...
    const status = String(messageData.status || "").toUpperCase();
    const reportId = String(messageData.reportId || "");

    let defaultTitle = "🔔 Building Care System";
    let defaultBody = "Ada pembaruan status laporan.";
    let vibration = [200, 100, 200];

    if (status === "OPEN") {
        defaultTitle = "🔵 Laporan Baru";
        defaultBody = reportId ? `Laporan ${reportId} telah diterima.` : "Ada laporan baru yang telah diterima.";
        vibration = [180, 80, 180];
    } else if (status === "PROGRESS") {
        defaultTitle = "🟠 Laporan Sedang Diproses";
        defaultBody = reportId ? `Laporan ${reportId} sedang ditangani.` : "Laporan sedang dalam proses penanganan.";
        vibration = [200, 100, 200, 100, 300];
    } else if (status === "DONE") {
        defaultTitle = "🟢 Pekerjaan Selesai";
        defaultBody = reportId ? `Laporan ${reportId} telah selesai ditangani.` : "Pekerjaan telah selesai ditangani.";
        vibration = [120, 70, 120, 70, 350];
    }
    ...
    const options = {
        ...
        tag: `bcs-${reportId || "notification"}-${status || "update"}`,
        renotify: true,
        vibrate: vibration,
        timestamp: Date.now(),
        data: {
        ...
        actions: [
            { action: "open", title: "Lihat Laporan" },
    ...
    event.waitUntil(
        ...
        .then(() => {
            console.log(`✅ [SW] Notification shown: ${status || "UPDATE"} ${reportId}`);
        })
        .catch(error => {
...
console.log(`✅ [SW] Service Worker ${CACHE_VERSION} FINAL PWA loaded`);
